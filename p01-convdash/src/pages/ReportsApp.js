...
import {postReport} from "../services/reports.service";
import Load from "../components/UI/Loader/Load";
// ----------------------------------------------------------------------
...
export default function DashboardApp() {
  ...
  const handleChange1 = async (event) => {
    ...
    if (!isLoading) {
      setIsLoading(true)
      await postReport({convNumber, valueFromDate, valueUntilDate})
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      const response = await axios.get("http://localhost:3001/categories");
      const categoriesNames = response.data.map(category => {
        return {full_name: category.full_name, id: category.id};
      })
      setCategories(categoriesNames);
    }
    fetchCategories();
    ...
    <Page title="Отчёты">
      <Container maxWidth="disabled">
        ...
        <Grid container spacing={1}>
          ...
          {pageVariantFill && !isLoading && <Grid item xs={12}>
            <AppReports loaddata={convNumber} datafrom={valueFromDate.customFormat("#MM#/#DD#/#YYYY# #hhhh#:#mm#")}
                        datato={valueUntilDate.customFormat("#MM#/#DD#/#YYYY# #hhhh#:#mm#")}/>
          </Grid>}
